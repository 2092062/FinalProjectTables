...
// for inserting data
            ...
            var insert_cmd="insert into cart (product_id,user_name) values (?,?)";
...
connect.query("select * from cart where user_name=? order by id desc limit 1",[user_name], function(err,result,fields){
    connect.on('error',(err)=>{
        console.log("[MySQL Error",err);
    });
...
app.post('/viewCart/',(req,res,next)=>{
...
// for inserting data
            ...
            connect.query("select * from cart where user_name=? ",[user_name], function(err,result,fields){
                connect.on('error',(err)=>{
                    console.log("[MySQL Error",err);
                });
